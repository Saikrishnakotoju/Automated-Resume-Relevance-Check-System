        ...
        if resume_files:
            ...
            for r_file in resume_files:
                ...
                for kw in missing_keywords:
                    highlighted_resume = highlighted_resume.replace(
                        kw,
                        f"<span style='background-color:#FF6347;color:white;padding:2px 4px;border-radius:3px'>{kw}</span>"
                    )
            ...
            for i in range(top_n):
                with st.expander(f"Resume: {df.iloc[i]['Resume']}"):
                    ...
                    badges_html = " ".join([
                        f"<span style='background-color:#FF6347;color:white;padding:3px 8px;border-radius:5px;margin:2px;'>{kw}</span>"
